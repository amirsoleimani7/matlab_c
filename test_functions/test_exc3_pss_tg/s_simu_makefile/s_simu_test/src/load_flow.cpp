...
void load_flow(MatrixXd bus, MatrixXd line, double tol, double iter_max, double acc ,string display ,double flag  , MatrixXd &bus_sol_1 , MatrixXd &line_sol_1 , MatrixXd &line_flow_1) {
    ...
    while(tap_it < tap_it_max && mm_chk == 1){
        ...
        MatrixXd test_bus(nbus,ncol);
        test_bus << bus_no ,V , ang, Pg , Qg,Pl,Ql,Gb,Bb, bus_type,qg_max,qg_min,bus.col(12),volt_max,volt_min;
        vector<Bus> busses = createBussesFromMatrix(test_bus);   //make it so it works in Y_Sparse ...
        NBus nBus_1(busses);
        vector<Line> lines_1 = createLinesFromMatrix(line);
        
        Y = Y_Sparse(&nBus_1,lines_1.data());
        ...
        if(conv_flag == 0){
            lftap(bus,V,volt_min,volt_max,line);        cout << "tap_it is : " << tap_it << "\n";
    ...
    MatrixXcd z = r.array() + (jay * rx.array());   
    ...
    {    
        ...
        complex<double> x_1 = VV(from_idx);  
        complex<double> x_22 = tps(k) * VV(to_idx);  
        complex<double> x_21 = VV(from_idx) - x_22;  
        complex<double> x_23 = VV(from_idx) * jay * chrg(k) / 2.0;  
        complex<double> x_2 = (x_21 * y(k)) + x_23;  
        x_2 = conj(x_2);  
    ...
    //     complex<double> x_1 = VV(to_idx);
    //     complex<double> x_21 = (VV(to_idx) - VV(from_idx)) / tps(k) * y(k);
    //     complex<double> x_23 = VV(to_idx) * jay * chrg(k) / 2.0;
    //     complex<double> x_2 = x_21 + x_23;
    //     x_2 = conj(x_2);
     ...
     for (int k = 0; k < nline; k++) 
    {
        ...
        complex<double> x_1 = VV(to_idx);
        // complex<double> x_21 = ((VV(to_idx) - VV(from_idx)) / tps(k));  //wrong 
        complex<double> x_21 = (VV(to_idx) - VV(from_idx) / tps(k));    //right

        complex<double> x_22 = x_21 * y(k);
        complex<double> x_23 = VV(to_idx) * jay * chrg(k) / 2.0;
        complex<double> x_2 = x_22 + x_23;
        x_2 = conj(x_2);
    ...
    for (int k = 0; k < nline; k++) 
    {
        ...
        complex<double> diff = VV(to_idx) - VV(from_idx) / tps(k);
        complex<double> part1 = diff * y(k);
        complex<double> part2 = VV(to_idx) * jay * chrg(k) / 2.0;
        complex<double> sum = part1 + part2;
        complex<double> conj_sum = conj(sum);

        MW_r(k, 0) = VV(to_idx) * conj_sum;
    }
