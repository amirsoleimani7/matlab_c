...

{
    ...
    for(int li = 0 ; li < n_st3 ; ++li){
        ...
        n(li , 0) = mac_int((int)exc_con(lj , 1) , 0); // probabbly needs to be 0-based .. // n is a matrix

        n_bus(li , 0) = bus_int((int)mac_con((int)n(li , 0) ,1)); // n_bus is a matrix

        Efd((int)n(li , 0) ,0) = vex((int)n(li , 0) ,0);

        if (Efd((int)n(li ,0) , 0) > exc_con(lj , 17)){ // check for max_lim ... 
            cout << "EXC_ST3: Efd exceeds maximum in initialization at " << n(li ,0) << "\n";
        }
        ...
        iterm((int)n(li ,0) , 0) = (pelect((int)n(li ,0), 0) - jay * qelect((int)n(li ,0), 0)) /
                (eterm((int)n(li ,0), 0) * exp(-jay * theta((int)n_bus(lj ,0), 0))) *
                mac_pot((int)n(li , 0) ,0);

        vep((int)n(li, 0) ,0) = eterm((int)n(li, 0), 0) * exp(-jay * theta((int)n(li, 0), 0)) *
            (exc_pot(lj, 0) + jay * exc_pot(lj, 1)); //not sure about the lj or n index in the exc_pot ... 

        ve((int)n(li, 0) ,0) = vep((int)n(li, 0) , 0) + jay * (exc_con(lj, 14) +
            (exc_pot(lj, 0) + jay * exc_pot(lj, 1)) *
            exc_con(lj, 15)) * iterm((int)n(li , 0) , 0); // the indexing is baed on the doucment not the matlab code 

        V_E = ve.cwiseAbs(); // not sure about this eather ... 
        ...
        I_N((int)n(li, 0) ,0) = exc_con(lj, 16) * fldcur((int)n(li  ,0 ), 0) / V_E((int)n(li , 0) ,0);

        
        


        if(I_N((int)n(li , 0) ,0) < 0.433){
            
            low_IN.conservativeResize(low_IN.rows() +1 , NoChange);
            low_IN(low_IN.rows() -1 , 0) = n(li , 0);
            F_EX((int)n(li , 0) ,0) = 1 -0.5771 * I_N((int)n(li ,0) , 0);
        }

        else if(I_N((int)n(li , 0) ,0) > 0.75){
            big_IN.conservativeResize(big_IN.rows() +1 , NoChange);
            big_IN(big_IN.rows() -1 , 0) = n(li , 0);
            F_EX((int)n(li , 0) ,0) = 1.708 * (1 - I_N((int)n(li , 0) ,0));
        }

        else{ // SOME THING BETWEEN .433 < I < .75 ...
            mid_IN.conservativeResize(mid_IN.rows() +1 , NoChange);
            mid_IN(mid_IN.rows() -1 , 0) = n(li , 0);
            F_EX((int)n(li ,0) ,0) = pow((.75 * (1 - pow(I_N((int)n(li ,0) ,0 ) , 2))) , .5); // make sure of this one ... 
        }
        
        if(F_EX((int)n(li ,0) ,0) <= 0){
            cout << "the F_EX(" << (int)n(li ,0)  << ",0)" << "\n";
        }

        V_B(lj, 0) = V_E((int)n(li ,0) ,1) * F_EX((int)n(li ,0) ,1);
        ...
        V_I((int)n(li , 0) ,0) = V_A(lj , 0) / exc_con(lj , 11);

        if(V_I((int)n(li ,0) ,0) > exc_con(lj , 9)) {
            cout << "cEXC_ST3: V_I above maximum in initialization at\n ";
        }

        if(V_I((int)n(li ,0) ,0) < exc_con(lj , 10)){
            cout << "EXC_ST3: V_I below minimum in initialization at\n";
        }


        exc_pot(lj ,2) = eterm((int)n(li ,0) ,0) + V_I((int)n(li , 0) ,0);
        V_TR(lj , 0) = eterm((int)n(li , 0) , 0);
        R_f(lj , 0) = 0;
    }
